"""
Mirror the curriculum CRUD changes from admin manage.tsx to teacher manage.tsx
"""
...
# The teacher manage.tsx uses the same curriculum JSX patterns - patch identically

# ---- 1. Add CRUD state variables ----
...
# ---- 2. Add CRUD handlers - inject before loadCourseData ----
# Find the point right after existing curriculum handlers and before the useEffects  
OLD_HANDLER_ANCHOR = '''    useEffect(() => {
        if (id) {
            loadCourseData();
        }
    }, [id]);'''

NEW_HANDLERS_PLUS_EFFECT = '''    const handleDeleteVideo = async (lessonId: number) => {
...
content = content.replace(OLD_HANDLER_ANCHOR, NEW_HANDLERS_PLUS_EFFECT)

# ---- 3. Update curriculum JSX - module header ----
...
print("Teacher manage.tsx updated with CRUD!")
