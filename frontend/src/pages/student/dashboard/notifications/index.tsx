        ...
        <StudentLayout title="Notifications">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Stay updated with your latest academic alerts.</p>
                </div>
                <button className="px-4 py-2 text-sm font-semibold text-primary-500 hover:bg-primary-500/10 rounded-xl transition-all border border-primary-500/20">
                    Mark all as read
                </button>
            </div>
            ...
            <div className="bg-dark-surface/50 backdrop-blur-md border border-dark-border rounded-2xl overflow-hidden shadow-xl">
                {loading ? (
                    <div className="p-20 text-center text-slate-500 flex flex-col items-center">
                        ...
                        <p className="font-medium">Loading your news...</p>
                    </div>
                    ...
                    <div className="p-20 text-center text-slate-500 flex flex-col items-center">
                        ...
                        <h3 className="text-xl font-bold text-dark-text mb-2">All caught up!</h3>
                        <p className="max-w-xs mx-auto">You have no new notifications at the moment. Check back later for updates.</p>
                    </div>
                ) : (
                    <div className="divide-y divide-dark-border/50">
                        {notifications.map((notif) => (
                            <div key={notif.id} className={`p-6 hover:bg-white/5 transition-all flex gap-5 group ${!notif.read ? 'bg-primary-500/5' : ''}`}>
                                ...
                                {!notif.read && (
                                    <div className="w-2.5 h-2.5 rounded-full bg-primary-500 shadow-[0_0_10px_rgba(37,99,235,0.5)] self-center"></div>
                                )}
                            </div>
                        ))}
                    </div>
